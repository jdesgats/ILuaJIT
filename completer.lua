-- Completion handling
-- Based on lua-rlcompleter by Patrick Rapin and Reuben Thomas
...
-- callback to set the character appended at the end of match (if only one)
M.final_char_setter = function(char) end

-- Completers are coroutines called by completion engine to generate possible matches.
-- Each possible value has to be yielded as string and completion engine matches it
-- with current buffer.
-- There is two contexts: 
--     * string: to complete literal strings, called with the begin of string
--     * value: to complete fields, called with a value (not necessarily a table) and spearator (`.`, `:` or `[`)
M.completers = { string = { }, value = { } }

-- Table fields completor
...
table.insert(M.completers.value, function(t, sep)
  ...
  if mt and type(mt.__index) == "table" then
    return M.completers.value[1](mt.__index, sep) -- use regular table completor on metatable
  end
end)
